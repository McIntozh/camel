...
public class AS2ClientConnection {
    ...
    public AS2ClientConnection(String as2Version, String userAgent, String clientFqdn, String targetHostName,
        ...
        try (AS2BHttpClientConnection testConnection = new AS2BHttpClientConnection("test", 8 * 1024)) {
            Socket socket;
            //          if ("https".equals(targetTransportProtocol)) {
            HttpCoreContext ctx = HttpCoreContext.create();
            ctx.setTargetHost(targetHost);
            socket = SSLConnectionSocketFactory.getSocketFactory().createSocket(ctx);
            //          } else {
            //            socket = new Socket(targetHost.getHostName(), targetHost.getPort());
            //          }
            testConnection.bind(socket);
        }
